...
def queryquicklook(name, region, minquantity):
     ...
     moop = cur.execute('SELECT typeid FROM typeid WHERE type=?',(name,))
     typeid = moop.fetchone()[0]
     payload = {'typeid':typeid, 'regionlimit':regionid[region], 'setminQ':minquantity}
...
if __name__=="__main__":
     ...
     for x in regionid.keys():
          xorders = queryquicklook('Hardwiring - Zainou \'Deadeye\' ZGA100', x, 3)
     ...
     #print numberjumpsroute(sqlthing1[0], sqlthing2[0]) , (float(buy['price']) - float(sell['price'])), sell['vol_remain'], buy['vol_remain']
     #print (min(float(sell['vol_remain']), float(buy['vol_remain'])) * (float(buy['price']) - float(sell['price'])))/float(numberjumpsroute(sqlthing1[0], sqlthing2[0])),' ISK per jump'
     print sell
     print buy
     print numberjumpsroute(sqlthing1[0], sqlthing2[0])
